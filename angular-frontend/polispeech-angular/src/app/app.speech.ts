...
export class SpeechComponent {
    ...
    public aftercall() {
        ...
        if (this.chartEl2 && this.chartEl2.nativeElement) {
          opts.title.text = "Complexity"
          opts.series[0].name = "Average word size"
            opts.chart = {
                type: 'spline',
        ...
        setInterval(function() {
            ...
            if (me._chart) {
                ...
                var sum = 0
                var word_complexity = 0
                for (var j = 0; j < 6; j += 1) {

                    if (polarity[words[6 * (i+1) + j]] != null) {
                        word_complexity += words[6 * (i+1) + j].length
                        sum += polarity[words[6 * (i+1) + j]]
                    }
                }
                console.log(sum)

                me._chart['series'][0].addPoint([(new Date()).getTime(), sum], true, true);
                me._chart2['series'][0].addPoint([(new Date()).getTime(), word_complexity], true, true);

                i = i + 1

                if((i-1)*6>words.length){
